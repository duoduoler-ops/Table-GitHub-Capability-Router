...
**当前版本为 [v0.5.0](https://github.com/duoduoler-ops/Table-GitHub-Capability-Router/releases/tag/v0.5.0)**，包含写入保护、使用记录、证据复核和可选 Hook。下载这个版本的源码即可使用这些功能，下面的说明也按这个版本编写。完整变更见 [CHANGELOG.md](CHANGELOG.md)。

| 版本 | 重点 | 用起来有什么变化 |
| --- | --- | --- |
| v0.5.0 | 写入保护、使用证据和可选 Hook | 旧草稿不能直接覆盖新内容；可追踪每次使用、发现证据过期或文件变化，并检查未完成事项 |
...
v0.5.0 新增的使用记录回答四个问题：**用的是哪个版本、在哪个项目里用、结果是否通过、后续事项是否处理完。** 旧记录里写过“健康”，不会被直接当作这次任务成功的证据。
...
**Hook 适合补上容易忘的提醒。** 本版提供 Codex 和 Grok Build 的可选示例，只检查已登记的未完成使用：
