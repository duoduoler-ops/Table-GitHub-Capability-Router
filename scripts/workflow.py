...
def semantic_project_router(records: Iterable[tuple[Path, dict[str, str], str]]) -> str:
    trigger_labels = {
        "high_confidence": "high confidence / 自动建议",
        "gated": "gated / 条件建议",
        "explicit_only": "explicit only / 仅点名建议",
    }
    rows = []
    for path, data, _ in sorted(records, key=lambda item: item[1]["id"]):
...
def validate_semantic_uniqueness(
    ...
    for _, data, _ in records:
        ...
        for example in project_semantic_examples(data):
            owners.setdefault(example.casefold(), []).append(data.get("id", "<missing-id>"))
    return [
        f"Duplicate semantic example across projects: {example!r} -> {sorted(set(project_ids))}"
        for example, project_ids in sorted(owners.items())
...
def collect_validation(root: Path) -> tuple[list[str], dict]:
    ...
    expected: dict[Path, str] = {
        root / "AGENT-ROUTER.md": replace_template(
            ...
            {"CLIENT_PROFILE": config["client_profile"], "LANGUAGE": config["language"]},
        )
    }
...
def normalized_semantic_metadata(
    examples: list[str] | None, trigger_level: str | None, negative_routing: str | None
) -> tuple[str, str, str]:
    ...
    return SEMANTIC_EXAMPLE_SEPARATOR.join(cleaned), trigger_level, negative
...
def command_project_routing(args: argparse.Namespace) -> dict:
    ...
    examples, trigger_level, negative_routing = normalized_semantic_metadata(
        args.semantic_example, args.trigger_level, args.negative_routing
    )
...
def command_project_transition(args: argparse.Namespace) -> dict:
    ...
    semantic_args_supplied = bool(args.semantic_example or args.trigger_level or args.negative_routing)
    if new in SEMANTIC_ELIGIBLE_STATUSES and (
        old not in SEMANTIC_ELIGIBLE_STATUSES or semantic_args_supplied
    ):
        semantic_examples, trigger_level, negative_routing = normalized_semantic_metadata(
            args.semantic_example, args.trigger_level, args.negative_routing
        )
